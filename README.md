...
The input-quality summary remains available when blank IDs or invalid measures block comparison. It reports missing cells, duplicate ID groups, exact duplicate extra rows, and column changes, with per-report counts and source record references. Missing values in other columns prompt review but do not block calculation. Counts overlap and are not a quality score. Missing selected columns are marked as not checked. The fictional “reports needing review” sample demonstrates corrections without changing or deleting source rows. A short business-question introduction explains why report quality should be checked before interpreting a performance change.

The quality checklist download remains available when comparison is blocked and includes all flagged record references, source values, priorities and next steps. It is a local, formula-safe CSV; overlapping issues are deliberately retained. Aggregate overflow remains a comparison check, not a checklist check. The summary prioritizes required fixes, mobile result navigation wraps, and a rejected file explicitly leaves the previously loaded reports unchanged.
