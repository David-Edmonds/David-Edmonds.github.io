...
export default function InquiryDraft(){
 ...
 const href=`mailto:davidedmondsc@gmail.com?subject=${encodeURIComponent(topics[topic])}&body=${encodeURIComponent(body)}`;
 async function copy(){try{await navigator.clipboard.writeText('davidedmondsc@gmail.com');setCopied('Email address copied.');}catch{setCopied('Copy the address shown below: davidedmondsc@gmail.com');}}
 return <section className="inquiry shell" id="inquiry" aria-labelledby="inquiry-title"><div><span>START WITH THE QUESTION</span><h2 id="inquiry-title">Tell me what<br/>needs to work better.</h2><p>A short description is enough to start. We can then discuss the data, audience, deliverables and scope before agreeing on the work.</p><ul><li>The decision or recurring report you need to improve.</li><li>Your current tools and the people who use the output.</li><li>Any timing or scope constraints.</li></ul><p>Keep the first message high-level; there’s no need to attach confidential data.</p></div>
