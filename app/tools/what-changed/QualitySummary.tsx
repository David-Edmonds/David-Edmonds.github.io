...
export default function QualitySummary({ quality, comparisonError, names, expanded, onReview, onInspect, canInspect }: Props) {
  ...
  return <section className={`panel input-quality ${status}`} aria-labelledby="input-quality-title">
    ...
    <p>{blocked ? 'Correct blank IDs and invalid measures, or review your column selection. No comparison totals are shown until the inputs can be compared.' : status === 'review' ? 'The totals can be calculated, but repeated IDs, missing values or column changes need a closer look.' : 'The selected IDs and measures passed these checks. This does not verify business accuracy or completeness.'}</p>
